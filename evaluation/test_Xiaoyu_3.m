...
cd '/Users/girinl/Desktop'
name1 = 'audio_orig.wav';
[x, fs] = audioread(name1);
name2 = 'audio_recon.wav';
[y, fs] = audioread(name2);
...
w = sin(pi*((0:(N-1)).' + 0.5)/N);      % sine window
w = ones(N,1);
...
% TD-RMSE
RMSE_TD_frame = sqrt(diag(e_mat'*e_mat)/N); % this vector contains RMSE values calculated in the TD for each frame

% FD-RMSE
RMSE_FD_frame = sqrt(diag(E'*E)/N^2); % this vector contains RMSE values calculated in the FD for each frame
...
% first way to average RMSE_TD_frame across frames
RMSE_TD_frame_mean_1 = mean(RMSE_TD_frame);

% second way to average RMSE_TD_frame across frames
RMSE_TD_frame_mean_2 = sqrt(sum(RMSE_TD_frame.^2*N)/N/M);