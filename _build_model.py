...
from utils import get_logger, SpeechSequencesFull, SpeechSequencesRandom
...
class BuildDKF(BuildBasic):
    ...
    def build(self):
        ...
        # Build model
        self.model = DKF(x_dim=self.x_dim, z_dim=self.z_dim, activation=self.activation,
...
def build_model(config_file='config_default.ini', training=True):
    ...
    if model_name == 'VAE':
        ...
        model_class = BuildDKF(cfg, training)
...
if __name__ == '__main__':
    model_class = build_model('config/cfg_dkf.ini', False)
